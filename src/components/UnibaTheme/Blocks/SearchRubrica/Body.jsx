...
import {
  SearchRubricaBackgroundUser,
  SearchRubricaBackground,
...
const Body = ({ block, sections }) => {
  ...
  return (
    <div className="public-ui searchRubrica">
      {block?.image ? (
        <SearchRubricaBackgroundUser image={block.image} />
      ) : (
        <SearchRubricaBackground />
      )}
      <div className="container">
        <div className="searchContainer d-flex w-100">
          {block.title && (
            <h2 className="search-rubrica-title mb-4">{block.title}</h2>
          )}
